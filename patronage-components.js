...
const Field = ({ label, name, type = "text", autocomplete = "", wide = false, placeholder = "" }) => `
  <div class="patron-field ${wide ? "patron-field--wide" : ""}"><label for="patron-${name}">${label}</label><input id="patron-${name}" name="${name}" type="${type}" ${autocomplete ? `autocomplete="${autocomplete}"` : ""} placeholder="${placeholder}" required /></div>`;
...
const PaymentPanel = () => `
  <section class="flow-panel flow-panel--payment" data-flow-panel="2" aria-labelledby="payment-title" hidden>
    ...
    <div class="payment-layout">
      ...
      <aside class="order-summary" aria-labelledby="summary-title"><p>Übersicht</p><h3 id="summary-title">Ihre Patenschaft</h3><dl><div><dt>Stein</dt><dd data-summary-stone>–</dd></div><div><dt>Spende</dt><dd data-summary-amount>180,00 €</dd></div><div><dt>Name</dt><dd data-summary-name>–</dd></div><div><dt>Zahlungsart</dt><dd data-summary-payment>Überweisung</dd></div></dl><p class="order-summary__note">Prototyp – es wird keine Zahlung ausgelöst.</p></aside>
    </div>
...
const SuccessPanel = () => `
  <section class="flow-panel flow-panel--success" data-flow-panel="3" aria-labelledby="success-title" hidden>
    ...
    <div class="success-card"><span>Ihre Auswahl</span><strong data-success-card-stone></strong><small data-success-payment></small></div>
    <div class="button-row"><a class="button button--primary" href="/"><span>Zur Startseite</span></a><button class="button button--outline" type="button" data-restart><span>Weiteren Stein wählen</span></button></div>
  </section>`;

const Intro = () => `
  <section class="patron-hero" id="top" aria-labelledby="patron-page-title">
    <div class="patron-hero__copy">
      <h1 class="brush-title" id="patron-page-title"><span>Jeder Stein</span><span>zählt!</span></h1>
      <p>Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod tempor invidunt ut labore et dolore magna aliquyam erat.</p>
      <a class="button button--light" href="#patenschaft"><span>Stein auswählen</span></a>
    </div>
    <div class="patron-hero__visual"><img src="${asset("tower-sunset.jpg")}" alt="Freiburger Münsterturm im Abendlicht" /><span aria-hidden="true"></span></div>
  </section>`;
